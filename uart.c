...
void set_uart(void) 
{
    ...
    //TRISBbits.TRISB7 = 1; // TX pin is output
    ...
    //SPBRG = (((_XTAL_FREQ/64)/BAUDRATE) - 1);         // baud rate formula
    //TXSTAbits.BRGH  = 1;                              //fast baud rate
...
void uart_write_string(const unsigned char *string)
{
	while(*string) {
        ...
        string++;
                                 ...
                                 // increase pointer location to the next characterr
}
...
//Reading text
//The following function can be used to read a desired lenght of text or sequence of characters continuosly
